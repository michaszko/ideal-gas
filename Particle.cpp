...
void Particle::isWallHit(double deltatime) {
  ...
  if ((next_pos.x + 2 * this->shape.getRadius() > 800) || (next_pos.x < 0)) {
    // we are outside of side walls
    this->velocity.x = -this->velocity.x;
    this->shape.setFillColor(sf::Color(100, 250, 50));
  }

  if ((next_pos.y + 2 * this->shape.getRadius() > 600) || (next_pos.y < 0)) {
...
void Particle::isParticleHit(double deltatime, Particle &p) {
